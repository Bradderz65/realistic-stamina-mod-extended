...
public class RStaminaMod implements ModInitializer {
	...
	public static final String rStaminaModVersion = "1.6.1";
	...
	private static void snapshotDefaultsFromConfig() {
		Defaults.staminaGainRate = config.staminaGainRate;
		Defaults.staminaLossRate = config.staminaLossRate;
		Defaults.jumpStaminaCost = config.jumpStaminaCost;

		Defaults.enableWalkingStaminaDrain = config.enableWalkingStaminaDrain;
		Defaults.walkingStaminaLossRate = config.walkingStaminaLossRate;
		Defaults.enableWalkingStaminaRegen = config.enableWalkingStaminaRegen;
		Defaults.walkingStaminaRegenMultiplier = config.walkingStaminaRegenMultiplier;

		Defaults.enableEnergySystem = config.enableEnergySystem;
		Defaults.speedBasedEnergyDrain = config.speedBasedEnergyDrain;
		Defaults.energyLossRate = config.energyLossRate;
		Defaults.walkingEnergyLossRate = config.walkingEnergyLossRate;

		Defaults.exhaustionThreshold = config.exhaustionThreshold;
		Defaults.exhaustionRecoveryTicks = config.exhaustionRecoveryTicks;
		Defaults.showExhaustionWarning = config.showExhaustionWarning;

		Defaults.enableSpeedMultiplier = config.enableSpeedMultiplier;
		Defaults.maxSpeedMultiplier = config.maxSpeedMultiplier;
	}
	...
	public void onInitialize() {
		...
		// snapshot defaults before loading user config
		snapshotDefaultsFromConfig();
			...
			if (config.enableMiningStaminaGain && !player.isCreative() && !player.isSpectator()) {
				...
				if (!world.isClient()) {
					...
					if (playerState.miningBlockProgress >= config.miningBlocksPerStaminaGain) {
						...
						// Update player's max stamina and mark state as dirty
						playerState.updateMaxStamina();
						serverState.markDirty();
					}
				}
